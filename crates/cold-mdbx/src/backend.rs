...
use signet_cold::{
    BlockData, BlockTag, ColdResult, ColdStorage, ColdStorageError, HeaderSpecifier,
...
impl MdbxColdBackend {
    ...
    fn get_block_by_hash(
        ...
        let tx = self.env.tx()?;
        TableTraverse::<ColdBlockHashIndex, _>::exact(
    ...
    fn get_tx_location(
        ...
        let tx = self.env.tx()?;
        TableTraverse::<ColdTxHashIndex, _>::exact(&mut tx.new_cursor::<ColdTxHashIndex>()?, &hash)
            .map_err(MdbxColdError::from)
    }
}

fn to_cold_result<T>(result: Result<T, MdbxColdError>) -> ColdResult<T> {
    result.map_err(ColdStorageError::backend)
}
...
impl ColdStorage for MdbxColdBackend {
    async fn get_header(&self, spec: HeaderSpecifier) -> ColdResult<Option<Header>> {
        to_cold_result((|| {
            ...
            let tx = self.env.tx()?;
            TableTraverse::<ColdHeaders, _>::exact(&mut tx.new_cursor::<ColdHeaders>()?, &block_num)
                .map_err(MdbxColdError::from)
        })())
    }
    ...
    async fn get_transaction(
        ...
        to_cold_result((|| {
            ...
            let tx = self.env.tx()?;
            DualTableTraverse::<ColdTransactions, _>::exact_dual(
    ...
    async fn get_zenith_header(
        ...
        to_cold_result((|| {
            ...
            let tx = self.env.tx()?;
            TableTraverse::<ColdZenithHeaders, _>::exact(
        ...
        to_cold_result((|| {
            ...
            match spec {
                ZenithHeaderSpecifier::Number(n) => {
                    let header: Option<DbZenithHeader> =
                        TableTraverse::<ColdZenithHeaders, _>::exact(
                            &mut tx.new_cursor::<ColdZenithHeaders>()?,
                            &n,
                        )?;
                ...
                ZenithHeaderSpecifier::Range { start, end } => {
                    ...
                    if let Some((key, value)) =
                        KvTraverse::<_>::lower_bound(&mut cursor, key_bytes)?
                    {
    ...
    async fn truncate_above(&self, block: BlockNumber) -> ColdResult<()> {
        to_cold_result((|| {
            ...
            for (block_num, _) in &headers_to_remove {
                let mut cursor = tx.new_cursor::<ColdTransactions>()?;
                for item in
                    DualTableTraverse::<ColdTransactions, _>::iter_k2(&mut cursor, block_num)?
                {
